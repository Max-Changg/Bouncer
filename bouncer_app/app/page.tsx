...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting) {
          ...
          console.log('Added animate class to:', element.textContent?.slice(0, 20));
        } else {
          // Remove animate class to reset
          element.classList.remove('animate');
          console.log('Removed animate class from:', element.textContent?.slice(0, 20));
        }
      });
    }, observerOptions);
    ...
    console.log('Elements to observe:', elementsToObserve.map(el => el?.textContent?.slice(0, 20)));
    ...
    setTimeout(() => {
      const fallbackElements = document.querySelectorAll('.opacity-0.transform.translate-y-8');
      console.log('Fallback elements found:', fallbackElements.length);
      fallbackElements.forEach(element => {
        observer.observe(element);
        console.log('Observing fallback element:', element.textContent?.slice(0, 20));
      });
    }, 1000);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-black to-gray-700 relative overflow-hidden" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <section id="why-bouncer-section" className="relative z-10 py-12 px-8 sm:px-20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 opacity-0 transform translate-y-8 transition-all duration-800 ease-out" ref={benefitsTitleRef}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16">
            <div className="bg-black/30 backdrop-blur-sm rounded-lg p-6 border border-purple-500/20 opacity-0 transform translate-y-8 transition-all duration-800 ease-out" ref={benefitCard1Ref}>
            ...
            <div className="bg-black/30 backdrop-blur-sm rounded-lg p-6 border border-orange-500/20 opacity-0 transform translate-y-8 transition-all duration-800 ease-out" ref={benefitCard2Ref}>
            ...
            <div className="bg-black/30 backdrop-blur-sm rounded-lg p-6 border border-purple-500/20 opacity-0 transform translate-y-8 transition-all duration-800 ease-out" ref={benefitCard3Ref}>
              <div className="w-12 h-12 bg-purple-500/20 rounded-lg flex items-center justify-center mb-4">
                <svg className="w-6 h-6 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-gradient-to-r from-purple-500/10 to-orange-500/10 rounded-lg p-8 border border-purple-500/20 opacity-0 transform translate-y-8 transition-all duration-800 ease-out" ref={costAdvantageRef}>
